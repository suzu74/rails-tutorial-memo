...
* Relationshipsコントローラのcreateアクションと destroyアクションを単に元のプロフィールにリダイレクトしていた
    * つまり、ユーザーはプロフィールページを最初に表示
        * そしてユーザーをフォローし、その後すぐ元のページにリダイレクトされる
            * この際、元のページにリダイレクトされる必要はない
                * そのために使うのがAjax

* Ajaxを使えば、Webページからサーバーに「非同期」で、ページを移動することなくリクエストを送信することができる
    * WebフォームにAjaxを採用するのは当たり前になるほど普及されている技術
        * そのためRailsでも簡単に実装できるようになっている
            * `form_for ..., remote: true`を使う
                * そしてコントローラーの変更も行う
                    * リクエストの種類によって応答を場合分けするときは、respond_toメソッドを使う

* まだ、少しAjaxの実装で不十分な箇所がある
    * Ajaxリクエストを受信した場合は、Railsが自動的にアクションと同じ名前を持つJavaScript用の埋め込みRuby (.js.erb) ファイル (create.js.erbやdestroy.js.erbなど) を呼び出す

* Ajax全体の理解がイマイチなので、別途学習をする

##### 14.2.6 フォローをテストする

* テストする内容はフォローする、されることでcount数が変わること
    * Ajaxの場合の部分もテストする
...
* 現在のユーザーによってフォローされているユーザーに対応するユーザーidを持つマイクロポストを取り出し、同時に現在のユーザー自身のマイクロポストも一緒に取り出すこと

* まずはテストから書く
    * このテストで重要なことは、フィードに必要な３つの条件を満たすこと
...
* このフィードで必要なクエリについて
    * micropostsテーブルから、自分がフォローしているユーザーに対応するidを持つマイクロポストをすべて選択 (select) すること
        * whereをうまく使うことで実現
            * `Micropost.where("user_id IN (?) OR user_id = ?", following_ids, id)`のような感じにする
                * following_idsメソッドは、has_many :followingの関連付けをしたときにActive Recordが自動生成したもの
                    * 関連付けによって作成されるメソッドについては頭の片隅に入れておきたい

##### 14.3.3 サブセレクト

* 今のままのfeedの実装だとフォローしているユーザーが多いとアプリが遅くなる可能性がある
    * うまくスケールできるように修正していく

* 現状の問題点
    * following_idsでフォローしているすべてのユーザーをデータベースに問い合わせし、さらに、フォローしているユーザーの完全な配列を作るために再度データベースに問い合わせしている点
        * このような問題は、SQLのサブセレクト (subselect) を使うと解決できる
            * SQLも含め再度復習する
                * パフォーマンス的なことを考慮するのがなかなか難しい

#### 14.4 最後に

* 多くを学んできたがこれ以上に学ばないといけないことが沢山ある

##### 14.4.1 サンプルアプリケーションの機能を拡張する

* Railsアプリケーションに何らかの機能を実装していて困ったときは、RailsガイドやRails APIをチェック
    * つい上記のようなドキュメントを避けているので意識して参考にしたい
...
* ルーティングにおいて、ネストさせた(member, colloction)
* whereメソッドを使うと、柔軟で強力なデータベースへの問い合わせが作成できる
* フォローしているユーザーのマイクロポスト一覧を表示させた

* 本章で理解がイマイチだと感じた箇所
    * 最終章ということもあり多くの部分が難しく感じた
        * Ajaxやパフォーマンスを考慮したfeedメソッドの実装
        * has_many の複雑なオプションを指定すること
            * おそらくしっかりと設計から考えることで見えてくるものが変わる気がする
    * 全体的に再度復習が必要と感じた